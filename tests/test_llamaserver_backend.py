...
async def test_successful_completion_returns_backend_response():
    ...
    backend = LlamaCPPServerBackend(
        base_url="http://llamaserver.local:8080", timeout=5.0, transport=transport
    )
...
async def test_read_timeout_raises_backend_timeout_error_immediately_without_retry():
    ...
    transport = httpx.MockTransport(handler)
    backend = LlamaCPPServerBackend(
        base_url="http://llamaserver.local:8080", timeout=5.0, max_retries=2, transport=transport
    )
...
async def test_classified_failure_is_logged_with_backend_host(caplog):
    ...
    transport = httpx.MockTransport(handler)
    backend = LlamaCPPServerBackend(
        base_url="http://llamaserver.local:8080", timeout=5.0, max_retries=1, transport=transport
    )

    with caplog.at_level("ERROR"), pytest.raises(BackendTimeoutError):
        await backend.complete(_request())

    assert "http://llamaserver.local:8080" in caplog.text
    assert "timed out" in caplog.text.lower()


def test_backend_id_reflects_configured_host():
    backend = LlamaCPPServerBackend(base_url="http://llamaserver.local:8080", timeout=5.0)

    assert backend.backend_id == "http://llamaserver.local:8080"


def test_connect_timeout_defaults_to_10_seconds_independent_of_the_gap_timeout():
    backend = LlamaCPPServerBackend(base_url="http://llamaserver.local:8080", timeout=120.0)
...
def test_connect_timeout_is_configurable_independent_of_the_gap_timeout():
    backend = LlamaCPPServerBackend(
        base_url="http://llamaserver.local:8080", timeout=120.0, connect_timeout=3.0
    )
...
async def test_check_health_reports_unhealthy_when_backend_is_unreachable():
    ...
    transport = httpx.MockTransport(handler)
    backend = LlamaCPPServerBackend(
        base_url="http://llamaserver.local:8080", timeout=5.0, max_retries=0, transport=transport
    )
...
async def test_list_models_returns_the_loaded_model_id():
    def handler(request):
        ...
        return httpx.Response(
            ...
            json={
                "data": [
                    {
                        "id": "/models/qwen2.5-coder-14b-instruct-q4_k_m.gguf",
                        "object": "model",
                    }
                ],
                "object": "list",
    ...
    assert models == ["/models/qwen2.5-coder-14b-instruct-q4_k_m.gguf"]
...
async def test_model_is_forwarded_unchanged():
    # Unlike LMStudioBackend, there is no alias-resolution layer here (Decision B: llama-server
    # processes are one-model-per-port, so multi-instance load balancing happens at the node
