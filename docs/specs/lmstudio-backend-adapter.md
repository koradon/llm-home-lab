...
[OpenAI-compatible API gateway](openai-compatible-api-gateway.md), so the gateway can serve
...
- Log each backend call with backend id/host, latency, and outcome (success / classified
  failure), so requests are traceable per the [program plan](../plans/orchestrator-program.md)'s
  M1 telemetry goal.
...
Keep scenarios in a sibling Gherkin file: `docs/specs/lmstudio-backend-adapter.feature`.

## Related

- Plan: [orchestrator-program](../plans/orchestrator-program.md) (M1 — LM Studio backend
  adapter)
- Spec: [openai-compatible-api-gateway](openai-compatible-api-gateway.md) (defines the
...
- Acceptance: `docs/specs/lmstudio-backend-adapter.feature`
