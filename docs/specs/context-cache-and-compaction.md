...
second backend after [failover](failover-and-health-policy.md) — doesn't re-run the assembly
work. This delivers the module only, the same way
[session-manager-core](session-manager-core.md) shipped `SessionManager` without wiring
...
- `ContextCache` is size-bounded (`max_entries`, default 128) with least-recently-used eviction,
  ...
  (see [workspace-state-capture](workspace-state-capture.md)).
...
Keep scenarios in a sibling Gherkin file: `docs/specs/context-cache-and-compaction.feature`.

## Related

- Spec: [session-manager-core](session-manager-core.md) — the `Session`/`StoredMessage`/`Summary`
...
- Spec: [failover-and-health-policy](failover-and-health-policy.md) — the failover-retry scenario
  motivating the cache (reassembling the same session against a fallback backend).
- Plan: [context-cache-and-compaction](../plans/context-cache-and-compaction.md)
- Plan: [orchestrator-program](../plans/orchestrator-program.md) (M3 — context cache and
  compaction)
- Issue: `.plan/milestones/m3-routing-and-reliability/issues/issue-003-context-cache-and-compaction.md`
  (#9)
- Acceptance: `docs/specs/context-cache-and-compaction.feature`
...
- Gateway adoption — how `/v1/chat/completions` would call `SessionManager.read_session` and
  ...
  [session-manager-core](session-manager-core.md)'s deferred decision, not resolved here.
